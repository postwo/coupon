...
class AsyncCouponIssueService2Test extends TestConfig {

    @Autowired
    AsyncCouponIssueService2 sut;
    ...
    void issue_3() {
        ...
        Coupon coupon = Coupon.builder()
                ...
                .build();
        couponJpaRepository.save(coupon);
        redisTemplate.opsForSet().add(getIssueRequestKey(coupon.getId()), String.valueOf(userId)); // 발급 받은걸로 처리
        // when & then
        CouponIssueException exception = Assertions.assertThrows(CouponIssueException.class, () -> {
            sut.issue(coupon.getId(), userId);
        });
